...
mlrmodel1 = lm(formula = sales ~ price + promotion, data=omni)  #we can give without names but with sequence and we can without seq stating the name 
...
# *** shows they are strongly relation their is statistically significance there is f values means at least one variable is dependant 

#in multiple regrssion model we use adjusted r values as we overfit as it puts penality by incereasing more variable # 74% variation in sales values depands on price & promtion 
range(omni$sales)
...
# by above cammand get values of price = -53 so if we keeping promotion constant if we incerease price one unit slaes change by 1 unit 
...
cbind(ndata1, Predict=predict(mlrmodel1, newdata=ndata1 ))
