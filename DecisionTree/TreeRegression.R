...
summary(Mer_Sales)
names(Mer_Sales)    #annual sales try to predict or regress
...
Index = sample(x = 1:nrow(Mer_SalesUncapped), size = 0.7*nrow(Mer_SalesUncapped))  #taking the rollnos 
...
#Me = mean[Mer_SalesTestUncapped$Annual_Sales]

#method -1 bcoz exclude merchant id 
CartFullModel
...
#how to prone the tree  where the cp values is least 
ptree= prune (CartFull , cp= 0.01000)
rpart.plot(x = CartFullModel, type = 4,fallen.leaves = T, cex = 1.0)
...
#the model which give min RMSE value
