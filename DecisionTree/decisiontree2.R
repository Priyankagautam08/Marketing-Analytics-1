...
# there is only split at left bcoz of less significance by gini index

testdata = data.frame(gender=c('M','M','F','F'),married=c('Married','Single','Married','Single'))
testdata
(p2=predict(fit2, newdata= testdata , type='vector'))
(p2=predict(fit2, newdata= testdata , type='class'))


#to make plot more attractive read rplot document 
...
#proning bcoz we dont want overfit our tree
head(student1)
...
pfit=  prune(fit2, cp=0.010) # from cptable  
printcp(fit2, digits = getOption("digits") - 5)
printcp(fit2)
pfit
rpart.plot(pfit)
