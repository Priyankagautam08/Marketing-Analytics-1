...
library(gsheet)
library(dplyr)
url = 'https://docs.gdgoogle.com/spreadsheets/d/1PWWoMqE5o3ChwJbpexeeYkW6p4BHL9hubVb1fkKSBgA/edit#gid=2051155174'
...
table(df1$gender)
apply(lapply(df1[factorcols],factor,ordered=TRUE), table)  #Method1
...
df1$dob = as.Date.character(df1$dob, '%d-%b-%y')
...
df1 %>% slice(1:n())
df1 %>% slice(1:10)
slice(df1, n()-fffh10: n())  #different way

names(df1)
#defaults to the last variable in the tbl
df1 %>% grou_by(gender)vcg %>% top_n(2)  #Rank not data
# top_n(n = 5, wt = x)
